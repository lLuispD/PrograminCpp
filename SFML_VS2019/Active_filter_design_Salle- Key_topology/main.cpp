...
#define HEIGHT_WINDOW   675.f       // ALTO 
#define WIDTH_WINDOW    1200.f      // ANCHO

int main(void) {


    sf::RenderWindow window(sf::VideoMode(WIDTH_WINDOW , HEIGHT_WINDOW), "Diseño de filtros activos topologia MFB - 2° orden");

    // Usamos punteros inteligentes para evitar fugas de memoria
    std::vector <std::string> FilterList = { "Filtro Paso Bajo", "Filtro Paso Alto", "Filtro Pasa Banda"};

    // Seleccion Filtro
    auto drpdw_listFiltersType = std::make_unique<UI::Dropdown>(FilterList, 340, (WIDTH_WINDOW / 13) * 1, 30, 150);
    ...
    */
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            drpdw_listFiltersType->handleEvent(event);
        ...
        // Lista Filtros
        window.draw(*drpdw_listFiltersType);
        // Ganancia
